...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // Auto-generate today's medicines from active schedules on load
        await generateDailyMedicines();
      } catch (err) {
        console.error('Failed to auto-generate today\'s doses:', err);
      }
      ...
      try {
        ...
        console.error('Failed to fetch today\'s doses:', err);
      }
      ...
      try {
        ...
        console.error('Failed to fetch week\'s doses:', err);
      }
    }
    loadData();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Room Environment — Live Sensor Data */}
      <RoomMonitor />
        ...
        <div className="space-y-2">
          ...
          {todayDoses.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-gray-300 mt-1">Add schedules in the Schedule tab to see doses here</p>
            </div>
          )}
        </div>
      </div>
