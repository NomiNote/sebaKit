// Schedule page — list schedules, add new medications + schedules, delete.
// CRUD via Supabase instead of Go backend.
...
export default function SchedulePage() {
  ...
  const grouped = schedules.reduce<Record<string, { name: string; items: typeof schedules }>>((acc, s) => {
    if (!acc[s.medication_id]) {
      acc[s.medication_id] = { name: s.medication_name, items: [] };
    }
    acc[s.medication_id].items.push(s);
    return acc;
  }, {});
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-sage-500 hover:bg-sage-600 text-white text-sm font-medium px-4 py-2 rounded-xl shadow-sm transition-all active:scale-95"
        >
          + Add
        </button>
      </div>
          ...
          <div className="divide-y divide-cream-100">
            {items.map((s) => (
              <div key={s.id} className="px-4 py-3 flex items-center justify-between">
                <div>
                  <span className="font-medium text-sage-600 text-sm">{to12h(s.time_of_day)}</span>
      ...
      {showModal && (
        <AddModal
          ...
          onAdd={async (medId, times, daysOfWeek, startDate, endDate) => {
            await createMultipleSchedules(medId, times, daysOfWeek, startDate, endDate);
            // Auto-generate today's dose instances so they appear on Dashboard immediately
            await generateDailyMedicines().catch(console.error);
...
function AddModal({
  ...
  onClose: () => void;
  onAdd: (medId: string, times: string[], daysOfWeek: string, startDate: string, endDate: string | null) => Promise<void>;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onAdd(finalMedId, times, days.join(','), startDate, endDate || null);
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-[380px] bg-white rounded-3xl p-6 shadow-2xl animate-slideUp max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading || (isNewMed && !newMedName.trim()) || (!isNewMed && !medId) || days.length === 0 || times.length === 0}
...
function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-[280px] w-full mx-4 animate-slideUp">
...
/** Convert "HH:MM:SS" or "HH:MM" (24h) → "h:mm AM/PM" */
