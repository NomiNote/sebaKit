...
void updateReedSwitch() {
  ...
  if ((millis() - lastReedChangeMs) >= REED_DEBOUNCE_MS) {
    if (lastStableReedState != lastRawReedState) {
      ...
      Serial.println(lastStableReedState == HIGH ? "HIGH (magnet removed)" : "LOW (magnet present)");

      // Only complete alert on LOW→HIGH (magnet physically removed)
      bool magnetRemoved = (previousState == LOW && lastStableReedState == HIGH);
...
void logPeriodicStatus() {
  ...
  Serial.print(" Reed=");
  Serial.println(lastStableReedState == HIGH ? "HIGH (no magnet)" : "LOW (magnet on)");
}
